...
<c:if test="${addrList != null}">
	<c:forEach var="list" items="${addrList }">
		<c:set var="address">
			${list.getSido() }${list.getSigungu() }${list.getYubmyundong() }${list.getRi() }${list.getRoadname() }${list.getBuildingname() }
		</c:set>
		<tr>
			<td align="center">${list.getZipcode() }</td>
			<td colspan="3">
				<a class="addressA" href="#" onclick="checkPostClose('${list.getZipcode() }','${address}')">${address}</a>
			</td>
		</tr>
	</c:forEach>
</c:if>
