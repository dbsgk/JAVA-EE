...
<form name="form" method="post" action="/miniProject/member/checkPost.do">
<table>
	<tr >
	...
	<c:forEach var="addrDTO" items = "${addrList }" varStatus="i">
		<c:set var = "zipcode1">${addrDTO.getZipcode() }</c:set>
		...
		<tr>
			...
			<c:when test = "${i.count % 2 == 1 }">
				<td class="fieldNames">
					<a onclick="selectAddr('${zipcode1 }', '${new_addr }')" style="cursor:pointer;">${zipcode1 }</a>
				</td>
				<td colspan="3" class="shadow">
					<div class="newAddr">
					<a onclick="selectAddr('${zipcode1 }', '${new_addr }')" style="cursor:pointer;">
						${new_addr } 
					</a>
					</div>
					<div class="oldAddr">
